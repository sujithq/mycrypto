...
- Real holdings can be entered row by row or pasted using an asset ID or symbol, `quantity`, `investedAmount`, and optional `buyDate` and `buyTimestamp` fields. Store `buyTimestamp` as an ISO-8601 UTC instant ending in `Z`; the dashboard displays it in `Europe/Brussels`, including DST. Legacy `amount`, `cost`, and `actualCost` fields are accepted on import. Assets may appear more than once with different buy dates or timestamps. TARS AI (TAI) is included in the supported asset universe.
